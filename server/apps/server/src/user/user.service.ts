...
import type { UserLogin, UserRegister, Token, RefreshTokenPayload, UserUpdate } from '@nexura/common/user';
...
export class UserService {
  ...
  async uploadAvatar(file: Express.Multer.File) {
    ...
    const databaseUrl = `/${bucket}/${fileName}`//数据库url
    const previewUrl = `${baseUrl}://${this.configService.get('MINIO_ENDPOINT')}:${port}${databaseUrl}`
    //previewUrl->http://192.168.1.6:9000/avatar/1234567890-chenfeng.jpg
    //databaseUrl->/avatar/1234567890-chenfeng.jpg
